...
class DeepFuzzyLogger(logging.getLoggerClass()): # type: ignore
...
logging.setLoggerClass(DeepFuzzyLogger)
...
log_level_from_env: str = os.environ.get("DEEPFUZZY_LOG", "2")
try:
  log_level_from_env_int: int = int(log_level_from_env)
  logger.setLevel(LOG_LEVEL_INT_TO_STR[log_level_from_env_int])
  logger.info("Setting log level from DEEPFUZZY_LOG: %d", log_level_from_env_int)
except ValueError:
  print("$DEEPFUZZY_LOG contains invalid value `%s`, "
        "should be int in 0-6 (debug, trace, info, warning, error, external, critical).",
        log_level_from_env)
  exit(1)
except KeyError:
  print("$DEEPFUZZY_LOG is in invalid range, should be in 0-6 "
        "(debug, trace, info, warning, error, external, critical).")
  exit(1)

__all__ = ["DeepFuzzyLogger", "LOG_LEVEL_INT_TO_STR", "LOG_LEVEL_INT_TO_LOGGER", "LOG_LEVEL_DEBUG",
