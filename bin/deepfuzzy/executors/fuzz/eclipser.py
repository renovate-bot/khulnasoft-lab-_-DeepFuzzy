...
from deepfuzzy.core import FuzzerFrontend, FuzzFrontendError
...
class Eclipser(FuzzerFrontend):
  ...
  def compile(self) -> None: # type: ignore
    ...
    for consistency.
    """
    lib_path: str = "/usr/local/lib/libdeepfuzzy.a"

    flags: List[str] = ["-ldeepfuzzy"]
  ...
  def cmd(self):
    ...
    # get deepfuzzy args and remove "-- binary"
    deepfuzzy_args = self.build_cmd([], input_symbol='eclipser.input')
    binary_index = deepfuzzy_args.index('--')
    deepfuzzy_args.pop(binary_index)
    deepfuzzy_args.pop(binary_index)
