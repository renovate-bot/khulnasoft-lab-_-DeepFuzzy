...
class Angora(FuzzerFrontend):
  ...
  def parse_args(cls) -> None:
    parser: argparse.ArgumentParser = argparse.ArgumentParser(
      description="Use Angora as a backend for DeepFuzzy.")
  ...
  def compile(self) -> None: # type: ignore
    ...
    taint_path: str = "/usr/local/lib/libdeepfuzzy_taint.a"
    L.debug("Static library path: %s", taint_path)

    taint_flags: List[str] = ["-ldeepfuzzy_taint"]
    ...
    fast_path: str = "/usr/local/lib/libdeepfuzzy_fast.a"
    L.debug("Static library path: %s", fast_path)

    fast_flags: List[str] = ["-ldeepfuzzy_fast"]
    if self.compiler_args:
