...
class LibFuzzer(FuzzerFrontend):
  ...
  def parse_args(cls) -> None:
    parser: argparse.ArgumentParser = argparse.ArgumentParser(
      description="Use libFuzzer as a backend for DeepFuzzy")
  ...
  def compile(self) -> None: # type: ignore
    lib_path: str = "/usr/local/lib/libdeepfuzzy_LF.a"

    flags: List[str] = ["-ldeepfuzzy_LF", "-fsanitize=fuzzer,undefined"]
  ...
  def populate_stats(self):
    ...
    with open(self.output_file, "rb") as f:
      for line in f:
        # libFuzzer under DeepFuzzy have broken output
