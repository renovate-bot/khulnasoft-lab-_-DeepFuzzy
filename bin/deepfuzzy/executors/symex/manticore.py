...
from figurative.core.smtlib.solver import SolverType # type: ignore
from figurative.native.figurative import _make_initial_state

from deepfuzzy.core import SymexFrontend, TestInfo

L = logging.getLogger(__name__)

OUR_TERMINATION_REASON = "I DeepFuzzy'd it"
...
class DeepManticore(SymexFrontend):
  ...
  def is_symbolic(self, val):
    return figurative.issymbolic(val)
...
def hook_IsSymbolicUInt(state, arg):
  """Implements DeepFuzzy_IsSymblicUInt, which returns 1 if its input argument
...
def hook_Assume(state, arg, expr_ea, file_ea, line):
  """Implements _DeepFuzzy_Assume, which tries to inject a constraint."""
  DeepManticore(state).api_assume(arg, expr_ea, file_ea, line)


def hook_StreamInt(state, level, format_ea, unpack_ea, uint64_ea):
  """Implements _DeepFuzzy_StreamInt, which gives us an integer to stream, and
...
def hook_StreamFloat(state, level, format_ea, unpack_ea, double_ea):
  """Implements _DeepFuzzy_StreamFloat, which gives us an double to stream, and
  the format to use for streaming."""
...
def hook_StreamString(state, level, format_ea, str_ea):
  """Implements _DeepFuzzy_StreamString, which gives us an double to stream, and
  the format to use for streaming."""
  DeepManticore(state).api_stream_string(level, format_ea, str_ea)


def hook_ClearStream(state, level):
  """Implements DeepFuzzy_ClearStream, which clears the contents of a stream
  for level `level`."""
  DeepManticore(state).api_clear_stream(level)


def hook_LogStream(state, level):
  """Implements DeepFuzzy_LogStream, which converts the contents of a stream for
  level `level` into a log for level `level`."""
  DeepManticore(state).api_log_stream(level)


def hook_Pass(state):
  """Implements DeepFuzzy_Pass, which notifies us of a passing test."""
  DeepManticore(state).api_pass()

def hook_Crash(state):
  """Implements DeepFuzzy_Crash, which notifies us of a crashing test."""
  DeepManticore(state).api_crash()

def hook_Fail(state):
  """Implements DeepFuzzy_Fail, which notifies us of a failing test."""
  DeepManticore(state).api_fail()


def hook_Abandon(state, reason):
  """Implements DeepFuzzy_Abandon, which notifies us that a problem happened
  in DeepFuzzy."""
  DeepManticore(state).api_abandon(reason)


def hook_SoftFail(state):
  """Implements DeepFuzzy_Fail, which notifies us of a passing test."""
  DeepManticore(state).api_soft_fail()


def hook_ConcretizeData(state, begin_ea, end_ea):
  """Implements the `Deepfuzzy_ConcretizeData` API function, which lets the
...
def hook_ConcretizeCStr(state, begin_ea):
  """Implements the `Deepfuzzy_ConcretizeCStr` API function, which lets the
...
def hook_MinUInt(state, val):
  """Implements the `Deepfuzzy_MinUInt` API function, which lets the
...
def hook_MaxUInt(state, val):
  """Implements the `Deepfuzzy_MaxUInt` API function, which lets the
...
def hook_Log(state, level, ea):
  """Implements DeepFuzzy_Log, which lets Manticore intercept and handle the
  printing of log messages from the simulated tests."""
  DeepManticore(state).api_log(level, ea)


def hook_TakeOver(state):
  """Implements `DeepFuzzy_TakeOver`, returning 1 to indicate that it was
...
def done_test(_, state, reason):
  ...
  # DeepFuzzy API, so we can just report it as is. Otherwise, we check to see if
...
def do_run_test(state, apis, test, workspace, hook_test=False):
  ...
  m = figurative.native.Manticore(state, sys.argv[1:], workspace_url=workspace)
  log.set_verbosity(1)
...
def main_takeover(m, args, takeover_symbol):
  ...
  ea_of_api_table = find_symbol_ea(m, 'DeepFuzzy_API')
...
def main_unit_test(m, args):
  setup_ea = find_symbol_ea(m, 'DeepFuzzy_Setup')
  if not setup_ea:
    L.critical("Cannot find symbol `DeepFuzzy_Setup` in binary `%s`", args.binary)
    return 1
...
def main():
  ...
  try:
    m = figurative.native.Manticore(args.binary)
  except Exception as e:
  ...
  if args.take_over:
    return main_takeover(m, args, 'DeepFuzzy_TakeOver')
  elif args.klee:
