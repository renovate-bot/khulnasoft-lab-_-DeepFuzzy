...
class IsSymbolicUInt(angr.SimProcedure):
  """Implements DeepFuzzy_IsSymbolicUInt, which returns 1 if its input argument
...
class SoftFail(angr.SimProcedure):
  """Implements DeepFuzzy_SoftFail, which notifies us of a failing test."""
  def run(self):
...
class ClearStream(angr.SimProcedure):
  """Implements DeepFuzzy_ClearStream, which clears the contents of a stream for
  level `level`."""
  def run(self, level):
...
class Log(angr.SimProcedure):
  """Implements DeepFuzzy_Log, which lets Angr intercept and handle the
...
class TakeOver(angr.SimProcedure):
    def run(self):
        """Do nothing, returning 1 to indicate that `DeepFuzzy_TakeOver()` has
...
def hook_apis(args, project, run_state):
  ...
  # use it.
  ea_of_api_table = find_symbol_ea(project, 'DeepFuzzy_API')
...
def main_take_over(args, project, takeover_symbol):
  ...
  # Concretely execute up until `DeepFuzzy_TakeOver`.
...
def main_unit_test(args, project):
  setup_ea = find_symbol_ea(project, 'DeepFuzzy_Setup')
  if not setup_ea:
  ...
  # Concretely execute up until `DeepFuzzy_Setup`.
  concrete_manager = angr.SimulationManager(
        project=project,
        active_states=[entry_state])
  ...
  try:
    ...
    L.critical("Execution never hit `DeepFuzzy_Setup` in binary `%s`", args.binary)
    return 1

  # Hook the DeepFuzzy API functions.
...
def main():
  ...
  if args.take_over:
    return main_take_over(args, project, 'DeepFuzzy_TakeOver')
  elif args.klee:
