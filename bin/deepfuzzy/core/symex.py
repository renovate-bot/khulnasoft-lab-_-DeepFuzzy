...
from deepfuzzy import (LOG_LEVEL_INT_TO_LOGGER,
                        LOG_LEVEL_TRACE, LOG_LEVEL_ERROR, LOG_LEVEL_CRITICAL)
from deepfuzzy.core.base import AnalysisBackend
...
class TestInfo(object):
  """Represents a `DeepFuzzy_TestInfo` data structure from the program, as
...
class SymexFrontend(AnalysisBackend):
  """Wrapper around a symbolic executor for making it easy to do common DeepFuzzy-
  specific things."""
  def __init__(self):
  ...
  def _read_test_info(self, ea):
    """Read in a `DeepFuzzy_TestInfo` info structure from memory."""
  ...
  def begin_test(self, info):
    ...
    # Create the symbols that feed API functions like `DeepFuzzy_Int`.
    symbols = []
  ...
  def report(self):
    ...
    if input_length > len(symbols):
      LOGGER.critical("Test overflowed DeepFuzzy_Input symbol array")
  ...
  def api_min_uint(self, arg):
    """Implements the `DeepFuzzy_MinUInt` API function, which returns the
    minimum satisfiable value for `arg`."""
    return self.concretize_min(arg, constrain=False)

  def api_max_uint(self, arg):
    """Implements the `DeepFuzzy_MaxUInt` API function, which returns the
    minimum satisfiable value for `arg`."""
    return self.concretize_max(arg, constrain=False)

  def api_is_symbolic_uint(self, arg):
    """Implements the `DeepFuzzy_IsSymbolicUInt` API, which returns whether or
  ...
  def api_assume(self, arg, expr_ea, file_ea, line):
    """Implements the `DeepFuzzy_Assume` API function, which injects a
  ...
  def api_pass(self):
    """Implements the `DeepFuzzy_Pass` API function, which marks this test as
  ...
  def api_crash(self):
    """Implements the `DeepFuzzy_Crash` API function, which marks this test as
  ...
  def api_fail(self):
    """Implements the `DeepFuzzy_Fail` API function, which marks this test as
  ...
  def api_soft_fail(self):
    """Implements the `DeepFuzzy_SoftFail` API function, which marks this test
    as having failed, but lets execution continue."""
    self.context['failed'] = True

  def api_abandon(self, arg):
    """Implements the `DeepFuzzy_Abandon` API function, which marks this test
  ...
  def api_log(self, level, ea):
    """Implements the `DeepFuzzy_Log` API function, which prints a C string
  ...
  def api_stream_int(self, level, format_ea, unpack_ea, uint64_ea):
    """Implements the `_DeepFuzzy_StreamInt`, which streams an integer into a
  ...
  def api_stream_float(self, level, format_ea, unpack_ea, double_ea):
    """Implements the `_DeepFuzzy_StreamFloat`, which streams an integer into a
    holding buffer for the log."""
    return self._api_stream_int_float(level, format_ea, unpack_ea,
                                      double_ea, float)

  def api_stream_string(self, level, format_ea, str_ea):
    """Implements the `_DeepFuzzy_StreamString`, which streams a C-string into a
    holding buffer for the log."""
    level = self.concretize(level, constrain=True)
    assert level in LOG_LEVEL_INT_TO_LOGGER
  ...
  def api_log_stream(self, level):
    """Implements DeepFuzzy_LogStream, which converts the contents of a stream
