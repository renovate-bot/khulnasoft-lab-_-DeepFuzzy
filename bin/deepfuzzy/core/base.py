...
from deepfuzzy import LOG_LEVEL_INT_TO_STR
...
class AnalysisBackend(object):
  ...
  enable us to build any DeepFuzzy-backing executor or auxiliary tool using a builder pattern
  for instantiation.
  """
  ...
  def parse_args(cls) -> Optional[argparse.Namespace]:
    ...
    if cls.parser is not None:
      ...
      parser = argparse.ArgumentParser(description="Use {} as a backend for DeepFuzzy".format(cls.NAME))
    ...
    if cls.compiler_exe:
      ...
      compile_group.add_argument("--compile_test", type=str,
        help="Path to DeepFuzzy test source for compilation and instrumentation by analysis tool.")
    ...
    parser.add_argument("binary", nargs="?", type=str,
      help="Path to the test binary compiled with DeepFuzzy to run under analysis tool.")
    ...
    parser.add_argument(
        ...
        help="Minimum DeepFuzzy log level to print (default: 2), 0-6 (debug, trace, info, warning, error, external, critical).")

    # DeepFuzzy-related options
    exec_group = parser.add_argument_group("DeepFuzzy Test Configuration")
    exec_group.add_argument(
      "--which_test", type=str,
      help="DeepFuzzy unit test to run (equivalent to `--input_which_test`).")

    exec_group.add_argument(
      "--target_args", default=[], nargs='*',
      help="Other DeepFuzzy flags to pass to harness before execution. Format: `a arg=val` -> `-a --arg1 val`.")
    ...
    # log level fixing
    if not os.environ.get("DEEPFUZZY_LOG"):
      ...
      logger = logging.getLogger("deepfuzzy")
      logger.setLevel(LOG_LEVEL_INT_TO_STR[_args["min_log_level"]])
    else:
      L.debug("Using log level from $DEEPFUZZY_LOG.")
  ...
  def build_from_config(config: str, allowed_keys: Optional[List[str]] = None, include_sections: bool = False) -> Union[Dict[str, Dict[str, Any]], Dict[str, Any]]:
    """
    Simple auxiliary helper that does safe and correct parsing of DeepFuzzy configurations. This can be used
    ...
    * Used externally as API for reasoning with configurations as part of auxiliary tools or test runners.
    ...
    reserved_sections: List[str] = [
      ...
      "internal"    # write-only by auxiliary tools, and should store anything not used by DeepFuzzy
    ]
