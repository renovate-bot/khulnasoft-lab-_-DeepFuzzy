...
from deepfuzzy.core.base import AnalysisBackend, AnalysisBackendError
...
class FuzzerFrontend(AnalysisBackend):
  """
  Defines a base front-end object for using DeepFuzzy to interact with fuzzers.
  """
  ...
  def __init__(self) -> None:
    ...
    self.stats_file: str = "deepfuzzy-stats.txt"
  ...
  def parse_args(cls) -> Optional[argparse.Namespace]:
    """
    Default base argument parser for DeepFuzzy frontends. Comprises of default arguments all
    ...
    if cls.parser:
      ...
      parser = argparse.ArgumentParser(description="Use {} fuzzer as a backend for DeepFuzzy".format(str(cls)))
    ...
    parser.add_argument(
      ...
      help="Show fuzzer-specific output (graphical interface) instead of deepfuzzy one.")
  ...
  def compile(self, lib_path: str, flags: List[str], _out_bin: str, env = os.environ.copy()) -> None:
    ...
    :param lib_path: path to DeepFuzzy static library for linking
    ...
    if not os.path.isfile(lib_path):
      raise FuzzFrontendError("No {}-instrumented DeepFuzzy static library found in {}".format(self, lib_path))
  ...
  def create_fake_seeds(self):
    if not self.input_seeds:
      self.input_seeds = mkdtemp(prefix="deepfuzzy_fake_seed")
  ...
  def build_cmd(self, cmd_list: List[Optional[str]], input_symbol: str = "@@") -> List[Optional[str]]:
    ...
    # initialize command with harness binary and DeepFuzzy flags to pass to it
    cmd_list.extend([
      "--", self.binary,
    ...
    # append any other DeepFuzzy flags
  ...
  def run(self, runner: Optional[str] = None, no_exec: bool = False, skip_argparse: bool = False):
    ...
    while run_fuzzer:
      ...
      try:
        if self.fuzzer_out:
          # disable deepfuzzy output
          L.info("Using fuzzer output.")
          L.setLevel("ERROR")
          self.proc = subprocess.Popen(command)

        else:
          L.info("Using DeepFuzzy output.")
  ...
  def post_exec(self):
    ...
    things like crash triaging, testcase minimization (ie with `deepfuzzy-reduce`), or any other manipulations
