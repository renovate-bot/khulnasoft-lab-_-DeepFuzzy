...
#ifdef DEEPFUZZY_TEST
#include <deepfuzzy/DeepFuzzy.h>
DeepFuzzy_EntryPoint(test_main) {
  const char *new_args[2];
  new_args[0] = "deepfuzzy";
  new_args[1] = DeepFuzzy_CStr_C(8, 0);

  DeepFuzzy_Assert(0 == old_main(2, new_args));
}

int main(int argc, const char *argv[]) {
  DeepFuzzy_InitOptions(argc, argv);
  return 0 == DeepFuzzy_Run();
}
