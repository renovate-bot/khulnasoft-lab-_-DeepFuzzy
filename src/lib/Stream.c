...
enum {
  DeepFuzzy_StreamSize = 1048576
};

/* Formatting options available to the streaming API. */
struct DeepFuzzy_StreamFormatOptions {
  /* int radix; */
  int hex;
  int oct;
  int show_base;
...
struct DeepFuzzy_Stream {
  int size;
  struct DeepFuzzy_StreamFormatOptions options;
  char message[DeepFuzzy_StreamSize + 2];
  char staging[32];
  char format[32];
  char unpack[32];
  union {
...
static struct DeepFuzzy_Stream DeepFuzzy_Streams[DeepFuzzy_LogFatal + 1] = {};
 ...
 *    >  Big endian
 */
static char DeepFuzzy_EndianSpecifier = '=';

/* Figure out what the Python `struct` endianness specifier should be. */
DEEPFUZZY_INITIALIZER(DetectEndianness) {
  static const int one = 1;
  if ((const char *) &one) {
    DeepFuzzy_EndianSpecifier = '<';  /* Little endian. */
  } else {
    DeepFuzzy_EndianSpecifier = '>';  /* Big endian. */
  }
}

/* Fills the `stream->unpack` character buffer with a Python `struct.unpack`-
 * compatible format specifier. */
static void DeepFuzzy_StreamUnpack(struct DeepFuzzy_Stream *stream, char type) {
  stream->unpack[0] = DeepFuzzy_EndianSpecifier;
  stream->unpack[1] = type;
  stream->unpack[2] = '\0';
}

/* Fill in the format for when we want to stream an integer. */
static void DeepFuzzy_StreamIntFormat(struct DeepFuzzy_Stream *stream,
...
static void CheckCapacity(struct DeepFuzzy_Stream *stream,
                          int num_chars_to_add) {
  if (0 > num_chars_to_add) {
    DeepFuzzy_Abandon("Can't add a negative number of characters to a stream.");
  } else if ((stream->size + num_chars_to_add) >= DeepFuzzy_StreamSize) {
    DeepFuzzy_Abandon("Exceeded capacity of stream buffer.");
  }
}
...
DEEPFUZZY_NOINLINE
void _DeepFuzzy_StreamInt(enum DeepFuzzy_LogLevel level, const char *format,
                       const char *unpack, uint64_t *val) {
  struct DeepFuzzy_Stream *stream = &(DeepFuzzy_Streams[level]);
  int size = 0;
  int remaining_size = DeepFuzzy_StreamSize - stream->size;
...
DEEPFUZZY_NOINLINE
void _DeepFuzzy_StreamFloat(enum DeepFuzzy_LogLevel level, const char *format,
...
DEEPFUZZY_NOINLINE
void _DeepFuzzy_StreamString(enum DeepFuzzy_LogLevel level, const char *format,
                          const char *str) {
  struct DeepFuzzy_Stream *stream = &(DeepFuzzy_Streams[level]);
  int remaining_size = DeepFuzzy_StreamSize - stream->size;
  int size = snprintf(&(stream->message[stream->size]),
...
void DeepFuzzy_StreamPointer(enum DeepFuzzy_LogLevel level, const void *val) {
  struct DeepFuzzy_Stream *stream = &(DeepFuzzy_Streams[level]);
  stream->format[0] = '0';
  stream->format[1] = 'x';
  stream->format[2] = '%';
  ...
  DeepFuzzy_StreamUnpack(stream, (sizeof(void *) == 8 ? 'Q' : 'I'));
  stream->value.as_uint64 = (uintptr_t) val;
  _DeepFuzzy_StreamInt(level, stream->format, stream->unpack,
                       &(stream->value.as_uint64));
}

#define MAKE_INT_STREAMER(Type, type, is_unsigned, pack_kind) \
    void DeepFuzzy_Stream ## Type(enum DeepFuzzy_LogLevel level, type val) { \
      struct DeepFuzzy_Stream *stream = &(DeepFuzzy_Streams[level]); \
      DeepFuzzy_StreamIntFormat(stream, sizeof(val), is_unsigned); \
      DeepFuzzy_StreamUnpack(stream, pack_kind); \
...
void DeepFuzzy_StreamCStr(enum DeepFuzzy_LogLevel level, const char *begin) {
  _DeepFuzzy_StreamString(level, "%s", begin);
}

/* Stream a some data in the inclusive range `[begin, end]` into the
 * stream's message. */
/*void DeepFuzzy_StreamData(enum DeepFuzzy_LogLevel level, const void *begin,
                       const void *end) {
  struct DeepFuzzy_Stream *stream = &(DeepFuzzy_Streams[level]);
  int remaining_size = DeepFuzzy_StreamSize - stream->size;
 ...
 * relevant data from `*val`, which may be symbolic, and defer the actual
 * formatting until later. */
void DeepFuzzy_StreamDouble(enum DeepFuzzy_LogLevel level, double val) {
  struct DeepFuzzy_Stream *stream = &(DeepFuzzy_Streams[level]);
  const char *format = "%f";  /* TODO(pag): Support more? */
  stream->value.as_fp64 = val;
  DeepFuzzy_StreamUnpack(stream, 'd');
  _DeepFuzzy_StreamFloat(level, format, stream->unpack, &(stream->value.as_fp64));
}

/* Clear the contents of the stream and don't log it. */
void DeepFuzzy_ClearStream(enum DeepFuzzy_LogLevel level) {
  struct DeepFuzzy_Stream *stream = &(DeepFuzzy_Streams[level]);
  if (stream->size) {
    stream->size = 0;
  }
...
void DeepFuzzy_LogStream(enum DeepFuzzy_LogLevel level) {
  struct DeepFuzzy_Stream *stream = &(DeepFuzzy_Streams[level]);
  if (stream->size) {
    stream->message[stream->size] = '\0';
    stream->message[DeepFuzzy_StreamSize] = '\0';
    DeepFuzzy_Log(level, stream->message);
    DeepFuzzy_ClearStream(level);
  }
}

/* Reset the formatting in a stream. */
void DeepFuzzy_StreamResetFormatting(enum DeepFuzzy_LogLevel level) {
  struct DeepFuzzy_Stream *stream = &(DeepFuzzy_Streams[level]);
  DeepFuzzy_MemScrub(&(stream->options), sizeof(stream->options));
}

static int DeepFuzzy_NumLsInt64BitFormat = 2;

/* `PRId64` will be "ld" or "lld" */
DEEPFUZZY_INITIALIZER(DeepFuzzy_NumLsFor64BitFormat) {
  DeepFuzzy_NumLsInt64BitFormat = (PRId64)[1] == 'd' ? 1 : 2;
}

/* Approximately do string format parsing and convert it into calls into our
 * streaming API. */
DEEPFUZZY_NOINLINE
static int DeepFuzzy_StreamFormatValue(enum DeepFuzzy_LogLevel level,
                                       const char *format,
...
#define READ_FORMAT_CHAR \
  ...
  if ('%' != ch) {
    DeepFuzzy_Abandon("Invalid format.");
    return 0;
  }
...
get_flag_char:
  ...
  switch (ch) {
    case '\0':
      DeepFuzzy_Abandon("Incomplete format (flags).");
      return 0;
    case '-':
    case '+':
    case ' ':
...
get_width_char:
  switch (ch) {
    case '\0':
      DeepFuzzy_Abandon("Incomplete format (width).");
      return 0;
    case '*':
      DeepFuzzy_Abandon("Variable width printing not supported.");
      return 0;
    case '0':
    case '1':
    case '2':
  ...
  get_precision_char:
    ...
    switch (ch) {
      case '\0':
        DeepFuzzy_Abandon("Incomplete format (precision).");
        return 0;
      case '*':
        DeepFuzzy_Abandon("Variable precision printing not supported.");
        break;
      case '0':
      case '1':
      case '2':
...
get_length_char:
  switch (ch) {
    case '\0':
      DeepFuzzy_Abandon("Incomplete format (length).");
      return 0;
    case 'L':
      long_double = 1;
      k += 1;  /* Overwrite the `L`. */
  ...
  if (!length) {
    length = 1;
  } else if (num_ls >= DeepFuzzy_NumLsInt64BitFormat) {
    length = 8;
  }
  ...
  switch(ch) {
    case '\0':
      DeepFuzzy_Abandon("Incomplete format (specifier).");
      return 0;
    ...
    case 'i':
      if (1 == length) {
        ...
        DeepFuzzy_Abandon("Unsupported integer length.");
      }
      goto common_stream_int;
    ...
    common_stream_int:
      DeepFuzzy_StreamUnpack(stream, extract);
      _DeepFuzzy_StreamInt(level, format_buf, stream->unpack,
                           &(stream->value.as_uint64));
      goto done;
    ...
    case 'A':
      ...
      DeepFuzzy_StreamUnpack(stream, 'd');
      _DeepFuzzy_StreamFloat(level, format_buf, stream->unpack,
                             &(stream->value.as_fp64));
      goto done;

    case 's': {
      const char *str = va_arg(va->args, const char *);
      _DeepFuzzy_StreamString(level, format_buf, str);
      goto done;
    }

    default:
      DeepFuzzy_Abandon("Unsupported format specifier.");
      return 0;
  }
done:
  if (!i) {
    DeepFuzzy_Abandon("Made no progress.");
  }
  return i;
}
 ...
 * following the `%d`. */
static char DeepFuzzy_Format[DeepFuzzy_StreamSize + 1];

/* Stream some formatted input. This converts a `printf`-style format string
 * into a */
void DeepFuzzy_StreamVFormat(enum DeepFuzzy_LogLevel level,
  ...
  char *begin = NULL;
  char *end = NULL;
  char *format = &(DeepFuzzy_Format[0]);
  int i = 0;
  char ch = '\0';
  char next_ch = '\0';
  size_t len = strlen(format_);

  if (len >= DeepFuzzy_StreamSize) {
    DeepFuzzy_Abandon("Format string is too long.");
  }

  /* Concretize the string format. */
  memcpy(format, format_, len);
  format[len] = '\0';
  DeepFuzzy_ConcretizeCStr(format);
  ...
  for (i = 0; '\0' != (ch = format[i]); ) {
    ...
    if ('%' == ch) {
      if ('%' == format[i + 1]) {
        ...
        DeepFuzzy_StreamCStr(level, begin);
        end[1] = next_ch;
        begin = NULL;
        end = NULL;
        i += 2;
        ...
        i += DeepFuzzy_StreamFormatValue(level, &(format[i]), &va);
      }
    } else {
      end = &(format[i]);
      i += 1;
...
void DeepFuzzy_StreamFormat(enum DeepFuzzy_LogLevel level,
                            const char *format, ...) {
  va_list args;
  va_start(args, format);
  DeepFuzzy_StreamVFormat(level, format, args);
  va_end(args);
}
