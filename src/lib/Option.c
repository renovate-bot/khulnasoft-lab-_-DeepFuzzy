...
static struct DeepFuzzy_Option *DeepFuzzy_Options = NULL;
int DeepFuzzy_OptionsAreInitialized = 0;

static const char DeepFuzzy_FakeSpace = ' ' | ((char) 0x80);

/* Copy of the option string. */
static int DeepFuzzy_OptionStringLength = 0;
static char DeepFuzzy_OptionString[kMaxOptionLength + 1] = {'\0'};
static const char *DeepFuzzy_OptionNames[kMaxNumOptions] = {NULL};
static const char *DeepFuzzy_OptionValues[kMaxNumOptions] = {NULL};

/* Defines a mapping between group enums and display strings */
static const char * ProcessGroupString(DeepFuzzy_OptGroup group) {
    return (const char *[]) {
...
static int CopyStringIntoOptions(int offset, const char *string,
  ...
  for (; offset < kMaxOptionLength && *string; ++string) {
    ...
    if (' ' == ch && replace_spaces) {
      ch = DeepFuzzy_FakeSpace;
    }
    DeepFuzzy_OptionString[offset++] = ch;
  }
  return offset;
}
...
static void TerminateOptionString(int length) {
  if (kMaxOptionLength <= length) {
    DeepFuzzy_Abandon("Option string is too long.");
  }
  DeepFuzzy_OptionString[length] = '\0';
  DeepFuzzy_OptionStringLength = length;
}
...
static int IsValidValueChar(char ch) {
  return (' ' < ch && ch <= '~') || ch == DeepFuzzy_FakeSpace;
}
...
static void ProcessOptionString(void) {
  char *ch = &DeepFuzzy_OptionString[0];
  char * const max_ch = &DeepFuzzy_OptionString[DeepFuzzy_OptionStringLength];
    ...
    switch (state) {
      case kInOption: {
        ...
        if (!IsValidOptionChar(ch_val)) {
          *ch = '\0';
          ...
          if ('=' == ch_val) {
            state = kSeenEqual;
          } else if (' ' == ch_val || DeepFuzzy_FakeSpace == ch_val) {
            state = kSeenSpace;
          } else {
            state = kElsewhere;
          }
      ...
      case kInValue:
        if (!IsValidValueChar(*ch)) {
          state = kElsewhere;
          *ch = '\0';
        } else if (DeepFuzzy_FakeSpace == *ch) {
          *ch = ' ';  /* Convert back to a space. */
        }
        break;

      case kSeenSpace:
        if (' ' == *ch || DeepFuzzy_FakeSpace == *ch) {
          *ch = '\0';
          state = kSeenSpace;
	} else if ('-' == *ch) {
	  state = kSeenDash;
          ...
          state = kInValue;
          DeepFuzzy_OptionValues[num_options - 1] = ch;
      ...
      case kSeenDash:
        if ('-' == *ch) {
          ...
          if (kMaxNumOptions <= num_options) {
            DeepFuzzy_Abandon("Parsed too many options!");
          }
          DeepFuzzy_OptionValues[num_options] = "";
          DeepFuzzy_OptionNames[num_options++] = ch + 1;
        } else {
          state = kElsewhere;
        }
        *ch = '\0';
...
static const char *FindValueForName(const char *name) {
  for (int i = 0; i < kMaxNumOptions && DeepFuzzy_OptionNames[i]; ++i) {
    if (!strcmp(DeepFuzzy_OptionNames[i], name)) {
      return DeepFuzzy_OptionValues[i];
    }
  }
  return NULL;
}
...
static void ProcessPendingOptions(void) {
  struct DeepFuzzy_Option *option = DeepFuzzy_Options;
  struct DeepFuzzy_Option *next_option = NULL;
...
void DeepFuzzy_InitOptions(int argc, ...) {
  va_list args;
  va_start(args, argc);
  ...
  ProcessOptionString();
  DeepFuzzy_OptionsAreInitialized = 1;
  ProcessPendingOptions();

  if (FLAGS_help) {
    DeepFuzzy_PrintAllOptions(argv[0]);
    exit(0);
  }
}
...
void DeepFuzzy_AddOption(struct DeepFuzzy_Option *option) {
  ...
  if (!option->next) {
    option->next = DeepFuzzy_Options;
    DeepFuzzy_Options = option;
  }
}
...
void DeepFuzzy_ParseBoolOption(struct DeepFuzzy_Option *option) {
  ...
  /* Alternative name, e.g. `--foo` vs. `--no_foo`. */
    ...
    if (alt_value != NULL) {
      if ('\0' != alt_value[0]) {
        DeepFuzzy_Abandon("Got an option value for a negated boolean option.");
      }
      *(option->has_value) = 1;
      *((int *) (option->value)) = 0;
    }
