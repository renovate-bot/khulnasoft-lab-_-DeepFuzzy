...
#ifndef SRC_INCLUDE_DEEPFUZZY_KLEE_H_
#define SRC_INCLUDE_DEEPFUZZY_KLEE_H_
...
static void klee_make_symbolic(void *addr, size_t nbytes, const char *name) {
  DeepFuzzy_SymbolizeData(addr, addr + nbytes);
}

static int klee_range(int begin, int end, const char *name) {
  return DeepFuzzy_IntInRange(begin, end);
}
...
DEEPFUZZY_NORETURN static void klee_silent_exit(int status) {
  exit(status);
}

DEEPFUZZY_NORETURN static void klee_abort(void) {
  abort();
}
...
static void klee_print_expr(const char *msg, ...) {
  /* KLEE debugging command, no DeepFuzzy equivalent. */
...
static void klee_warning(const char *message) {
  DeepFuzzy_Log(DeepFuzzy_LogWarning, message);
}
...
static void klee_prefer_cex(void *object, uintptr_t condition) {
  /* KLEE engine command, no DeepFuzzy equivalent. */
}
...
static KLEE_GET_VALUE(l, long) {
  if (sizeof(long) == sizeof(int)) {
    return DeepFuzzy_MinInt((int) val);
  } else {
    // TODO: We need a MinInt64 function.
    return DeepFuzzy_MinInt(val);
  }
}
...
#endif  /* SRC_INCLUDE_DEEPFUZZY_KLEE_H_ */
