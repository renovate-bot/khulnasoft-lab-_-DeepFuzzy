...
class CrashFuzzerTest(deepfuzzy_base.DeepFuzzyFuzzerTestCase):
  def run_deepfuzzy(self, deepfuzzy):
    def do_compile(tempdir, test_source_file):
      ...
      # run command
      (r, output) = logrun.logrun([deepfuzzy] + arguments, output_log_file, 360)
    ...
    def do_fuzz(tempdir, compiled_files):
      ...
      # run command
      (r, output) = logrun.logrun([deepfuzzy] + arguments, output_log_file, 
    ...
    with TemporaryDirectory(prefix="deepfuzzy_test_fuzzers_") as tempdir:
