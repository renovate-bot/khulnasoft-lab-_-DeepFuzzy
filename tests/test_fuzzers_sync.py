...
class CrashFuzzerTest(TestCase):
  def test_fuzzers_synchronization(self):
    def do_compile(fuzzer, tempdir, test_source_file):
      ...
      # run command
      proc = subprocess.Popen([f"deepfuzzy-{fuzzer}"] + arguments)
      proc.communicate()
    ...
    def do_fuzz(fuzzer, workspace_dir, sync_dir, compiled_files, output_from_fuzzer=None):
      ...
      """
      # prepare args
      output_dir = mkdtemp(prefix=f"deepfuzzy_{fuzzer}_", dir=workspace_dir)
    ...
    def crashes_found(fuzzer, output):
      ...
      Check if some crash were found assuming that
        fuzzer output is the deepfuzzy one (--fuzzer_out == False)
      """
      no_crashes = 0
    ...
    def do_sync_test(output_from_fuzzer=None):
      ...
      for fuzzer in fuzzers.keys():
        ...
        fuzzers[fuzzer]["stats_file"] = os.path.join(output_dir, "deepfuzzy-stats.txt")

      # import Frontend classes so we can use PUSH/PULL/CRASH dirs
      deepfuzzy_python = os.path.join(os.path.dirname(__file__), "bin", "deepfuzzy")
      print(f"Adding deepfuzzy python path: {deepfuzzy_python}.")
      sys.path.append(deepfuzzy_python)

      if "afl" in fuzzers:
        from deepfuzzy.executors.fuzz.afl import AFL
        fuzzers["afl"]["class"] = AFL
      if "angora" in fuzzers:
        from deepfuzzy.executors.fuzz.angora import Angora
        fuzzers["angora"]["class"] = Angora
      if "honggfuzz" in fuzzers:
        from deepfuzzy.executors.fuzz.honggfuzz import Honggfuzz
        fuzzers["honggfuzz"]["class"] = Honggfuzz
      if "eclipser" in fuzzers:
        from deepfuzzy.executors.fuzz.eclipser import Eclipser
        fuzzers["eclipser"]["class"] = Eclipser
      if "libfuzzer" in fuzzers:
        from deepfuzzy.executors.fuzz.libfuzzer import LibFuzzer
